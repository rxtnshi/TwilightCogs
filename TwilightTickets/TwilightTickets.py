...
class TwilightTickets(commands.Cog):
	...
	""" async def load_configs(self):
		...
		if guild:
			...
			self.modmail_access_role = await self.config.guild(guild).modmail_access_role()
			self.management_access_role = await self.config.guild(guild).management_access_role()
			self.appeal_team_role = await self.config.guild(guild).appeal_team_role()
			self.discord_staff_team_role = await self.config.guild(guild).discord_staff_role()
			self.scpsl_staff_team_role = await self.config.guild(guild).scpsl_staff_role()
			self.appeal_log_channel = await self.config.guild(guild).appeal_log_channel()
			self.ticket_categories = await self.config.guild(guild).ticket_categories()
			self.ticket_log_channel = await self.config.guild(guild).ticket_log_channel()
			self.panel_channel = await self.config.guild(guild).panel_channel()
